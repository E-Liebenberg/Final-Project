...
def add_remote(request):
    if request.method == 'POST':
        form = RemoteForm(request.POST, user=request.user)   # ✅
        if form.is_valid():
            remote = form.save()

            # ✅ handle optional patient assignment on create
            assigned_patient = form.cleaned_data.get('assigned_patient')
            if assigned_patient:
                Patient.objects.filter(remote=remote).update(remote=None)
                assigned_patient.remote = remote
        ...
        form = RemoteForm(user=request.user)                 # ✅
...
def edit_remote(request, remote_id):
    ...
    if request.method == 'POST':
        form = RemoteForm(request.POST, instance=remote, user=request.user)  # ✅
        if form.is_valid():
            remote = form.save()

            assigned_patient = form.cleaned_data.get('assigned_patient')

            # Unassign this remote from all patients, then (re)assign if provided
            Patient.objects.filter(remote=remote).update(remote=None)
            if assigned_patient:
                assigned_patient.remote = remote
                assigned_patient.save()
        ...
        form = RemoteForm(instance=remote, user=request.user)               # ✅

    return render(
        request,
        'remotes/add_edit_remote.html',
        {'form': form, 'remote': remote}
    )
